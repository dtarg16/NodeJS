...
const writeFile = () => new Promise((resolve, reject) => {
  ...
  fs.writeFile(`${__dirname}/stream-big-example04-writeFile.txt`, data, 'utf8', (err) => {
    if (err) reject(err);
    resolve();
  });
});

const file = fs.createWriteStream(`${__dirname}/stream-big-example04-createWriteStream.txt`);
